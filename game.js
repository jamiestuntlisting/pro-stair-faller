...
class PlayScene extends Phaser.Scene {
    ...
    drawCrewScene(tier, p) {
        ...
        let tilt = 0, scatter = 0, panic = 0, drop = 0, fly = 0;
        if (tier === 1) { tilt = Math.sin(p*20)*4*(1-p)*0.02; scatter = 0; }
        else if (tier === 2) { tilt = Math.min(p*3,1)*0.3; scatter = Math.min(p*3,1)*15; panic = 1; }
        else if (tier === 3) { tilt = Math.min(p*2,1)*1.4; scatter = Math.min(p*2,1)*30; panic = 2; }
        else if (tier === 4) { tilt = Math.min(p*3,1)*1.57; scatter = Math.min(p*2,1)*60; drop = Math.min(p*2,1)*25; panic = 2; }
        else if (tier >= 5) { tilt = Math.min(p*4,1)*1.57; scatter = Math.min(p*1.5,1)*120; drop = Math.min(p*2,1)*40; fly = Math.min(p*2,1); panic = 3; }

        const H = CONFIG.PERSON_HEIGHT;
        // All crew BEHIND camera (to the right — camera faces left toward player)
        this.drawCameraRig(g, cx, cy, tilt, drop);
        // Camera operator (right behind camera, peering through viewfinder)
        this.drawCrewPerson(g, cx + H*0.20, cy, 0x4a4a5a, false, false, scatter, panic, fly, 'lean_left');
        // Focus puller (next to operator)
        this.drawCrewPerson(g, cx + H*0.45, cy, 0x5a4a3a, false, false, scatter, panic, fly * 0.8, 'standing');
        // Director (furthest back, arms crossed, beret + monitor)
        this.drawCrewPerson(g, cx + H*0.72, cy, 0x2a2a3a, true, true, scatter, panic, fly, 'confident');
    }
    ...
    drawCrewPerson(g, baseX, baseY, bodyCol, hasBeret, hasMonitor, scatter, panic, fly, pose) {
        ...
        if (panic === 0) {
            ...
            g.lineStyle(H*0.025, skin, 1);
            g.beginPath();
            g.moveTo(shoulderX - H*0.08, shoulderY); g.lineTo(shoulderX - H*0.06, shoulderY - H*0.14);
            g.moveTo(shoulderX + H*0.08, shoulderY); g.lineTo(shoulderX + H*0.06, shoulderY - H*0.14);
            g.strokePath();
        } else {
            // Flailing
            const fl = Math.sin((this.crashAnimTime || 0) * 14) * H*0.06;
            g.lineStyle(H*0.025, skin, 1);
            g.beginPath();
            g.moveTo(shoulderX - H*0.08, shoulderY); g.lineTo(shoulderX - H*0.12 + fl, shoulderY - H*0.12 - fly*H*0.08);
            g.moveTo(shoulderX + H*0.08, shoulderY); g.lineTo(shoulderX + H*0.12 - fl, shoulderY - H*0.16 - fly*H*0.08);
            g.strokePath();
        }
    ...
    updateRolling(time, dt) {
        ...
        this.rollRotation += (dist / circ) * Math.PI * 2;
    ...
    onPlayerStopped() {
        ...
        this.scoreData = { distMeters: dm, isPerfect: Math.abs(this.playerWorldX - this.levelData.markX) < CONFIG.PERFECT_THRESHOLD_PX, crashed: false, healthCost: hc };
        this.showScore();
    }
    ...
    onPlayerCrashed() {
        ...
        // Place the player at the projected stop point
        this.playerWorldX = this.levelData.cameraX + simDist;
    ...
    updateStopped(time, dt) {
        ...
        if (this.crashTier > 0) { this.crashAnimTime += dt; this.drawCrewScene(this.crashTier, Math.min(this.crashAnimTime/1.0, 1)); }
    }
}
